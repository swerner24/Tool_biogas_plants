...
results_LCA = {
    sp: {
        ...
        "CHP_electricity_benefit_CO2": 0,
        "CHP_heat_benefit_CO2": 0,
        "AD_electricity_use_CO2": 0,
        "CHP_infrastructure_CO2": 0,
...
for sp, res_sp in results_per_species.items():
    ...
    heat_generated_GJ=biogas_yield_GJ*CHP_heat*(8000/8670)

    electricity_demand_AD_kWh=biogas_produced_m3*electricity_demand/1000/3.6
    heat_demand_AD_GJ=biogas_produced_m3*heat_demand/1000000

    #CO2 emissions
    electricity_demand_kgCO2=electricity_demand_AD_kWh * electricity_co2/1000

    #CO2 benefits
    heat_benefits_kgCO2=((heat_generated_GJ-heat_demand_AD_GJ)*external_heat_usage)*heat_substitution*1000
    electricity_benefits_kgCO2=(electricity_generated_GJ*1000/3.6)*electricity_co2/1000

    #share fermenter for heat to total heat
    share_fermenter=(heat_demand_AD_GJ/heat_generated_GJ)*100

    electricity_generated_GJ_chp [sp] = electricity_generated_GJ
    biogas_produced_m3_pw2 [sp]= biogas_produced_m3
    results_LCA[sp]["AD_electricity_use_CO2"] = electricity_demand_kgCO2
    results_LCA[sp]["CHP_heat_benefit_CO2"] = -heat_benefits_kgCO2
    results_LCA[sp]["CHP_electricity_benefit_CO2"] = -electricity_benefits_kgCO2

    #print(f"\n--- {sp} ---")
    #print("electricity_demand (kg CO2/t):", round(electricity_demand_kgCO2, 2))
    #print("heat_benefits (kg CO2/t):", round(heat_benefits_kgCO2, 2))
    #print("electrictiy_benefits (kg CO2/t):", round(electricity_benefits_kgCO2, 2))
    #print("Share fermenter heat to total heat (%):", round(share_fermenter, 2))
...
# Construction
#50kWel
CHP_infrastructure=26573.9764256915 #kgCO2 pro chp
CHP_maintenance= 1171.83084839318
CHP_schlupf=0.01
AD_infrastructure=42236.193476516

for sp, res_sp in results_per_species.items():

    electricity_production_per_lifetime_kWh=50*100000 #kWel * h
    kgCo2_per_kWh_CHP=(CHP_infrastructure+CHP_maintenance)/electricity_production_per_lifetime_kWh
    CHP_infrastructure_kgCO2= (electricity_generated_GJ_chp[sp]* kgCo2_per_kWh_CHP * 277.78)+( biogas_produced_m3_pw2[sp]*CH4_Biogas*CHP_schlupf*27*RHO_CH4)
    ...
    results_LCA[sp]["CHP_infrastructure_CO2"] = CHP_infrastructure_kgCO2
    results_LCA[sp]["AD_infrastructure_CO2"] = AD_infrastructure_kgCo2

    # print(f"\n--- {sp} ---")
    # print("Infrastructure CHP (kg CO2/t):", round(CHP_infrastructure_kgCO2, 2))
...
for sp in results_LCA:
    ...
    Construction_operation_CHP=(results_LCA[sp]["CHP_infrastructure_CO2"])

    Energy_benefits=(results_LCA[sp]["CHP_electricity_benefit_CO2"]
               + results_LCA[sp]["CHP_heat_benefit_CO2"])
    ...
    GWP_Total=Pre_storage+Transport+AD_Methane_losses+Construction_operation_AD+Construction_operation_CHP+ Energy_benefits+ Post_storage+Field_application
...
# Komponenten zurückschreiben (optional, aber praktisch)
    ...
    results_LCA[sp]["GWP_CHP_construction_operation"] = Construction_operation_CHP
...
cols = [
    ...
    "GWP_CHP_construction_operation",
    "GWP_poststorage",
...
def precompute_chp_lca_factors_for_climate(
    ...
    Returns:
      factors[species] = dict with:
        ...
        - plus optional components (same units)
    """
    days_pre_winter=days_pre_summer
    days_post_winter = 180
    SPECIES = ["Cattle", "Horses", "Sheep", "Goats", "Pigs", "Poultry"]

    # --- 1) PRE-STORAGE per tFM (your exact LCA logic, but tFM=1) ------------
    # Build systems per species with tFM=1 and compute emissions
    results_per_species_pre = {}
    for sp in SPECIES:
        species_tonnes = {sp: 1.0}
        ...
        res_pre = compute_daily_storage_emissions_multi(systems_sp, year=2025)
        results_per_species_pre[sp] = res_pre

    # Transport per tonne (your function)
    transport_co2e_per_t = transport_kgCO2e_per_tonne(distance_one_way_km=7.5)

    # --- 2) AD methane slip + energy benefits + infra (your exact LCA logic) --
    AD_EFF  = 0.85
    SLIP    = 0.0105
    CH4_Biogas = 0.625
    ...
    # For POST-STORAGE + FIELD, we need post-storage results too
    results_per_species_post = {}
    net_biomethane_m3 = {}
    methane_losses_kg = {}
    electricity_generated_GJ_chp = {}
    biogas_produced_m3_pw2 = {}
    ...
    for sp in SPECIES:
        res_pre = results_per_species_pre[sp]
        pre_ch4_kg = float(res_pre["total"]["ch4_kg_daily"].sum())
        pre_n2o_kg = float(res_pre["total"]["n2o_kg_daily"].sum())
        ...
        losses_kg = net_m3 * SLIP * RHO_CH4

        net_biomethane_m3[sp] = net_m3
        methane_losses_kg[sp] = losses_kg

        # --- energy calc (your logic)
        biogas_produced_m3 = (net_m3 - (losses_kg/RHO_CH4)) / CH4_Biogas
        biogas_yield_GJ = biogas_produced_m3 * CH4_Biogas * 35.883/1000
        ...
        electricity_demand_AD_kWh = biogas_produced_m3 * electricity_demand/1000/3.6
        heat_demand_AD_GJ = biogas_produced_m3 * heat_demand/1_000_000
        ...
        # --- infra (your logic)
        electricity_production_per_lifetime_kWh = 50 * 100000
        kgCo2_per_kWh_CHP = (CHP_infrastructure+CHP_maintenance)/electricity_production_per_lifetime_kWh

        CHP_infra_kgCO2 = (
            electricity_generated_GJ * kgCo2_per_kWh_CHP * 277.78
            + biogas_produced_m3 * CH4_Biogas * CHP_schlupf * 27 * RHO_CH4
        )

        kgCo2_per_kWh_AD = AD_infrastructure/(50*8000*20)
        AD_infra_kgCO2 = (electricity_generated_GJ * kgCo2_per_kWh_AD * 277.78)

        # --- POST-STORAGE (digestate) systems (your logic)
        remaining_m3 = max(biomethane_yield_ch_mix[sp] - pre_ch4_m3 - net_m3, 0.0)
        N_after = max(N_content[sp] - pre_n2o_Nkg, 0.0)
        N_after_AD_kgN[sp] = N_after
        ...
        res_post = compute_daily_storage_emissions_multi(systems_sp_post, year=2025)
        results_per_species_post[sp] = res_post

        post_ch4_kg = float(res_post["total"]["ch4_kg_daily"].sum())
        post_n2o_kg = float(res_post["total"]["n2o_kg_daily"].sum())
        post_n2o_N = post_n2o_kg * (28/44)

        # --- FIELD N2O (your logic)
        ...
        # --- COMPONENTS in kg CO2eq per t FM
        Pre_storage = pre_ch4_kg*GWP_CH4 + pre_n2o_kg*GWP_N2O
        Transport   = transport_co2e_per_t
        AD_slip     = losses_kg*GWP_CH4
        AD_const_op = electricity_demand_kgCO2 + AD_infra_kgCO2
        CHP_const   = CHP_infra_kgCO2
        Post_storage= post_ch4_kg*GWP_CH4 + post_n2o_kg*GWP_N2O
        Field_app   = N2O_field_kg*GWP_N2O

        # IMPORTANT:
        # In your table you store benefits as negative already.
        # If you want GWP_Total = sum(all components), then Benefits should be (already negative).
        Benefits = (-electricity_benefits_kgCO2) + (-heat_benefits_kgCO2)

        Total = (Pre_storage + Transport + AD_slip + AD_const_op + CHP_const
        ...
        factors[sp] = {
            ...
            "GWP_CHP_construction_operation": CHP_const,
